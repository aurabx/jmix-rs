...
impl AssertionManager {
    ...
    pub fn verify_assertion(
        ...
        let public_key_bytes = BASE64
            .decode(&assertion.signing_key.public_key)
        ...
        let field_values =
            temp_manager.extract_field_values(entity, manifest, &assertion.signed_fields)?;
    ...
    pub fn verify_directory_attestation(_attestation: &DirectoryAttestation) -> JmixResult<bool> {
...
mod tests {
    ...
    fn test_sender_assertion_creation() -> JmixResult<()> {
        ...
        assert!(result
            .assertion
            .signed_fields
            .contains(&"sender.name".to_string()));
        assert_eq!(
            result.assertion.expires_at,
            Some("2025-01-01T00:00:00Z".to_string())
        );
    ...
    fn test_assertion_verification() -> JmixResult<()> {
        ...
        // Create assertion
        let result = manager.create_sender_assertion(&entity, &manifest, None, None, None)?;

        // Verify assertion
        let verification =
            AssertionManager::verify_assertion(&result.assertion, &entity, &manifest)?;
    ...
    fn test_receiver_assertion_creation_and_verification() -> JmixResult<()> {
        ...
        let result =
            manager.create_receiver_assertion(&receiver, &manifest, idx, None, None, None)?;
        ...
        assert!(result
            ...
            .signed_fields
            .iter()
            .any(|f| f == &format!("receiver.{}.id", idx)));
    ...
    fn test_invalid_assertion_verification() -> JmixResult<()> {
        ...
        // Verify should fail
        let verification =
            AssertionManager::verify_assertion(&tampered_assertion, &entity, &manifest)?;
