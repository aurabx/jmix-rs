...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config: Config =
        serde_json::from_str(&config_content).expect("Failed to parse configuration JSON");
    ...
    let _signing_manager = JwsManager::generate_keypair(&private_key_path, &public_key_path)?;
    ...
    let (envelope, dicom_files) = builder
        .build_from_dicom(dicom_path, &config)
    ...
    let saved_files = builder
        .save_to_files(&envelope, &dicom_files, output_dir)
    ...
    if jws_parts.len() == 3 {
        ...
        if let Ok(header_bytes) =
            base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(jws_parts[0])
        {
