...
fn main() -> JmixResult<()> {
    ...
    if let Some(sender_assertion) = &envelope.manifest.sender.assertion {
        println!(
            "   ✓ Generated sender assertion with {} signed fields",
            sender_assertion.signed_fields.len()
        );
        println!(
            "   ✓ Sender assertion algorithm: {}",
            sender_assertion.signing_key.alg
        );
    } else {
    ...
    if let Some(requester) = &envelope.manifest.requester {
        if let Some(requester_assertion) = &requester.assertion {
            println!(
                "   ✓ Generated requester assertion with {} signed fields",
                requester_assertion.signed_fields.len()
            );
        } else {
    ...
    println!(
        "   ✓ Signed fields: {:?}",
        custom_assertion_result.assertion.signed_fields
    );
    println!(
        "   ✓ Key reference: {:?}",
        custom_assertion_result.assertion.key_reference
    );
    println!(
        "   ✓ Expires at: {:?}",
        custom_assertion_result.assertion.expires_at
    );
    ...
    match custom_verification {
        VerificationResult::Valid {
            ...
            println!(
                "   ✓ Signed {} fields including payload hash",
                signed_fields.len()
            );
        }
        VerificationResult::Invalid { reason } => {
