...
fn test_load_sample_config() {
    ...
    if !std::path::Path::new(config_path).exists() {
        println!(
            "Skipping test - sample config file not found: {}",
            config_path
        );
        return;
    }

    let content = fs::read_to_string(config_path).expect("Failed to read sample config file");

    let config: config::Config =
        serde_json::from_str(&content).expect("Failed to parse sample config");
...
fn test_load_sample_manifest() {
    ...
    if !std::path::Path::new(manifest_path).exists() {
        println!(
            "Skipping test - sample manifest file not found: {}",
            manifest_path
        );
        return;
    }

    let content = fs::read_to_string(manifest_path).expect("Failed to read sample manifest file");

    let manifest: types::Manifest =
        serde_json::from_str(&content).expect("Failed to parse sample manifest");
    ...
    assert_eq!(
        manifest.security.classification.as_deref(),
        Some("confidential")
    );
...
fn test_load_sample_metadata() {
    ...
    if !std::path::Path::new(metadata_path).exists() {
        println!(
            "Skipping test - sample metadata file not found: {}",
            metadata_path
        );
        return;
    }

    let content = fs::read_to_string(metadata_path).expect("Failed to read sample metadata file");

    let metadata: types::Metadata =
        serde_json::from_str(&content).expect("Failed to parse sample metadata");
    ...
    assert_eq!(
        studies.study_description.as_deref(),
        Some("CT Pulmonary Angiogram")
    );
...
fn test_load_sample_audit() {
    ...
    if !std::path::Path::new(audit_path).exists() {
        println!(
            "Skipping test - sample audit file not found: {}",
            audit_path
        );
        return;
    }

    let content = fs::read_to_string(audit_path).expect("Failed to read sample audit file");

    let audit: types::Audit = serde_json::from_str(&content).expect("Failed to parse sample audit");
    ...
    assert_eq!(
        audit.audit[1].to.as_ref().unwrap().id,
        "org:receiver.clinic"
    );
...
fn test_load_sample_files() {
    ...
    if !std::path::Path::new(files_path).exists() {
        println!(
            "Skipping test - sample files file not found: {}",
            files_path
        );
        return;
    }

    let content = fs::read_to_string(files_path).expect("Failed to read sample files file");

    let files: types::Files = serde_json::from_str(&content).expect("Failed to parse sample files");
...
fn test_validate_sample_files_with_schemas() {
    ...
    if let Ok(content) = fs::read_to_string("samples/sample_audit.json") {
        let audit: types::Audit =
            serde_json::from_str(&content).expect("Failed to parse sample audit");
    ...
    if let Ok(content) = fs::read_to_string("samples/sample_files.json") {
        let files: types::Files =
            serde_json::from_str(&content).expect("Failed to parse sample files");
...
fn test_config_to_jmix_types_conversion() {
    ...
    assert_eq!(
        manifest_sender.name,
        Some("Test Healthcare Organization".to_string())
    );
...
fn test_sample_data_consistency() {
    ...
    for file_path in &sample_files {
        if std::path::Path::new(file_path).exists() {
            ...
            let _json_value: serde_json::Value =
                serde_json::from_str(&content).expect(&format!("Invalid JSON in {}", file_path));
    ...
    assert!(
        found_files > 0,
        "No sample files found - this suggests a problem with test setup"
    );
