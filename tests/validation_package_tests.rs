...
fn validate_unencrypted_package_roundtrip() {
    ...
    let (envelope, dicom_files) = builder
        .build_from_dicom(&dicom_dir, &cfg)
        .expect("build from dicom");

    // Save envelope
    let _saved = builder
        .save_to_files_with_options_no_zip(&envelope, &dicom_files, &base_out, false, false)
